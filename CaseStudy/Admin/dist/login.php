...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['userName']) && isset($_POST['password'])) {
        ...
        $myPassword =  isset($_POST['password']) ? $_POST['password'] : null;
        $query = "SELECT * FROM dbquanlibanhang.user";
        $result = $pdo->query($query);
        $result = $result->fetchAll();
    }
    foreach ($result as $value) {
        if ($value['user_name'] == $myUserName && $value['password'] == $myPassword) {
            $_SESSION['login_user'] = $value['user_name'];
            break;
        }
    }
